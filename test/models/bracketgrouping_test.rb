...
class BracketgroupingTest < ActiveSupport::TestCase
  ...
  test "se_matchups" do
	assert  bracketgroupings(:bballdiv1).se_matchups(7) == [[[1, nil], [4, 5]], [[2, 7], [3, 6]]]
	assert  bracketgroupings(:bballdiv1).se_matchups(8) == [[[1, 8], [4, 5]], [[2, 7], [3, 6]]]
	assert  bracketgroupings(:bballdiv1).se_matchups(9) == [[[[1, nil], [8, 9]], [[4, nil], [5, nil]]], [[[2, nil], [7, nil]], [[3, nil], [6, nil]]]]
  end
  ...
  test "for_pair_champs" do
    assert_difference('Bracketcontest.count', 6) do
      matchups =  bracketgroupings(:bballdiv1).for_pair(bracketgroupings(:bballdiv1).se_matchups(7))
    end
  end
